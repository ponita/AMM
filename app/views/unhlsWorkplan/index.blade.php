...
<div>
	<ol class="breadcrumb">
	  ...
	  <li class="active">UNHLS Workplan</li>
	</ol>
</div>
...
<div class="panel panel-primary">
	<div class="panel-heading ">
		...
		UNHLS Workplan
		<div class="panel-btn">
			<a class="btn btn-sm btn-info" href="{{ URL::to("unhlsWorkplan/create") }}" >
		...
		<table class="table table-striped table-hover table-condensed search-table">
			...
			@foreach($departments as $key => $value)
				<tr @if(Session::has('activeunhlsWorkplan'))
					...
					<td>
           @if($value->thematicArea_id)
          {{ $value->thematicarea->name }}
        @endif
       </td>
					<td>{{ $value->name }}</td>
					...
					<!-- show the test category (uses the show method found at GET /departments/{id} -->
						<a class="btn btn-sm btn-success" href="{{ URL::to("unhlsWorkplan/" . $value->id) }}" >
							<span class="glyphicon glyphicon-eye-open"></span>
							{{ trans('messages.view') }}
						</a>

					<!-- edit this test category (uses edit method found at GET /unhlsWorkplan/{id}/edit -->
						<a class="btn btn-sm btn-info" href="{{ URL::to("unhlsWorkplan/" . $value->id . "/edit") }}" >
