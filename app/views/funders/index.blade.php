	...
	<div class="panel-body">
		<table class="table table-striped table-hover table-condensed search-table">
			...
			@foreach($funders as $key => $value)
				<tr @if(Session::has('activefunders'))
                            {{(Session::get('activefunders') == $value->id)?"class='info'":""}}
                        @endif
                        >
					...
					<!-- show the test category (uses the show method found at GET /testcategory/{id} -->
						<a class="btn btn-sm btn-success" href="{{ URL::to("funders/" . $value->id) }}" >
							<span class="glyphicon glyphicon-eye-open"></span>
							{{ trans('messages.view') }}
						</a>

					<!-- edit this test category (uses edit method found at GET /testcategory/{id}/edit -->
						<a class="btn btn-sm btn-info" href="{{ URL::to("funders/" . $value->id . "/edit") }}" >
